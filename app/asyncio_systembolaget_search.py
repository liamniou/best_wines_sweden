...
TIMEOUT = int(os.getenv("CHROME_TIMEOUT", 5000))
...
async def get_list_of_parsed_values_with_evaluate(page_object, selector_query, evaluate_function):
    resulting_list = []
...
async def perform_ladder_search(browser, query_text):
    ...
    try:
        ...
        search_page_with_results = await open_page_wait_selector(browser, url, ".css-1n00zeq")
...
def get_systembolaget_info_about_drink(url_encoded_string):
    return asyncio.get_event_loop().run_until_complete(asyncio.wait_for(find_in_systembolaget(url_encoded_string), 600))
